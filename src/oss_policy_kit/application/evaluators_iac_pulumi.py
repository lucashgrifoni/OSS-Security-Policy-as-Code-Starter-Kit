...
def _load_evidence(repo_root: Path) -> tuple[dict[str, Any] | None, EvalOutcome | None]:
    ...
    data = read_scanner_evidence(
        evidence, label="Pulumi IaC", regenerate_cmd="oss-policy-kit scan-pulumi", schema_prefix=_SCHEMA_PREFIX
    )
...
def _make_pulumi_evaluator(rule_id: str, summary: str) -> Callable[[Any], EvalOutcome]:
    def _eval(ctx: Any) -> EvalOutcome:
        ...
        if not files_scanned:
            blocked = absent_technology_outcome(data, technology="Pulumi", sources=sources)
        ...
        if count == 0:
            return EvalOutcome(
                ...
                reason=(
                    f"No {rule_id} findings detected across {len(files_scanned)} scanned Pulumi program(s)."
