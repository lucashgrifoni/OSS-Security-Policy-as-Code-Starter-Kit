...
| --- | --- | --- | --- |
| (root) | — | `--version/-V`, `--help/-h`, `--show-profiles/-sp` (**deprecated, use the `profiles` subcommand**) | Compatibility entry point; also accepts the same flags as `evaluate` for backward compat. |
